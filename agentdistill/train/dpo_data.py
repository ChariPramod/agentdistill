...
def pair_is_valid(pair: dict) -> tuple[bool, str]:
    ...
    # Neither side acts, so the text is the decision.
    if _normalized_text(chosen) == _normalized_text(rejected):
        return False, "chosen and rejected say the same thing"
    return True, ""
...
def filter_pairs(pairs: list[dict]) -> tuple[list[dict], dict[str, int]]:
    """Keep the usable pairs and count why the rest were dropped."""
    kept: list[dict] = []
